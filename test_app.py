...
def test_full_turn_and_player_switch(client):
    ...
    response = client.post('/api/score', json={'base_score': 20, 'multiplier': 1}) # P1 Dart 3: 20
    data = response.get_json()
...
def test_bust_rule_score_of_one(client):
    ...
    response = client.post('/api/score', json={'base_score': 20, 'multiplier': 1}) # Score would be 1, bust!
    data = response.get_json()
...
def test_around_the_world_logic(client):
    ...
    response = client.post('/api/score', json={'base_score': 5, 'multiplier': 1}) # Miss
    data = response.get_json()
...
def test_around_the_world_win(client):
    ...
    response = client.post('/api/score', json={'base_score': 25, 'multiplier': 1}) # Hit the bull
    data = response.get_json()
